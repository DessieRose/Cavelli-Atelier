        ...
        <div class="flex m-10 mb-px justify-between items-center">
            ...
            <div class="flex gap-3">
                <form method="POST" action="{{ route('products.destroy', $product) }}">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2.5 rounded-full transition-colors">
                        Delete Product
                    </button>
                </form>
                    ...
                    <x-form-section title="Name and Description">
                        <div class="flex flex-col gap-5">
                            ...
                            <div>
                                ...
                                <input class="input-field" type="text" id="name" name="name" value="{{ old('name', $product->name) }}">
                                @error('name')
                                <p>{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <fieldset>
                                ...
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    <div class="flex flex-col min-w-28 flex-1">
                                        ...
                                        <input class="border border-gray-200 rounded-lg px-3 py-2 bg-gray-50" type="number" id="height" name="height" value="{{ old('height', $product->height) }}" placeholder="H:">
                                    </div>

                                    {{-- width --}}
                                    <div class="flex flex-col min-w-28 flex-1">
                                        <label class="whitespace-nowrap text-sm text-gray-500 mb-1" for="width">Width (mm):</label>
                                        <input class="border border-gray-200 rounded-lg px-3 py-2 bg-gray-50" type="number" id="width" name="width" value="{{ old('width', $product->width) }}" placeholder="W:">
                                    </div>

                                    {{-- length --}}
                                    <div class="flex flex-col min-w-28 flex-1">
                                        <label class="whitespace-nowrap text-sm text-gray-500 mb-1" for="length">Length (mm):</label>
                                        <input class="border border-gray-200 rounded-lg px-3 py-2 bg-gray-50" type="number" id="length" name="length" value="{{ old('length', $product->length) }}" placeholder="L:">
                                    </div>

                                    {{-- weight --}}
                                    <div class="flex flex-col min-w-28 flex-1">
                                        <label class="whitespace-nowrap text-sm text-gray-500 mb-1" for="weight">Weight (kg):</label>
                                        <input class="border border-gray-200 rounded-lg px-3 py-2 bg-gray-50" type="number" id="weight" name="weight" value="{{ old('weight', $product->weight) }}" placeholder="WT:">
                                    </div>
                                </div>
                            </fieldset>
                    ...
                    <x-form-section title="Pricing">
                        <div class="flex gap-4">
                            ...
                            <div>
                                ...
                                <input class="input-field" type="number" id="price" name="price" step="0.01" value="{{ old('price', $product->price) }}">
                                @error('price')
                                <p>{{ $message }}</p>
                                @enderror
                            </div>
